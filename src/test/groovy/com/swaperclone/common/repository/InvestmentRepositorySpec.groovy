...
class InvestmentRepositorySpec extends RepositorySpec {
    ...
    def "test findByLoanId"() {
        given:
        Loan ln = prepareLn(debtorName: 'John', amount: BigDecimal.valueOf(1000), investorInterest: BigDecimal.valueOf(11),
        ...
        prepareInvestment(investor: user, amountToReceive: BigDecimal.valueOf(1100), loan: ln)

        when:
        Optional<Investment> i1 = investmentRepository.findByLoanId(-1L)
        Optional<Investment> i2 = investmentRepository.findByLoanId(ln.id)
    ...
    def "test findInvestmentStatuses"() {
        ...
        result.content[0].getPaidAmount() == BigDecimal.ZERO
        ...
        result.content[1].getPaidAmount() == BigDecimal.valueOf(210)
    }
